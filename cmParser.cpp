...
bool cmListFileParser::ParseFile(const char *filename)
{
	...
	if (bom != cmListFileLexer_BOM_None && bom != cmListFileLexer_BOM_UTF8) {
		cmListFileLexer_SetFileName(this->Lexer, nullptr, nullptr);
		std::cerr << "File starts with a Byte-Order-Mark that is not UTF-8." << std::endl;
		return false;
	}
				   ...
				   cmListFileArgument::Delimiter delim)
{
	...
	bool isError =
		(this->Separation == SeparationError || delim == cmListFileArgument::Bracket);
	std::ostringstream m;
